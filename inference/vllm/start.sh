...
./dp-inference-config --engine vllm config/config.json >"${runtime_tmp}"
chmod 600 "${runtime_tmp}"
mv -f -- "${runtime_tmp}" "${runtime_env}"
trap - EXIT HUP INT TERM

./load_offline_image.sh vllm

echo "正在启动 vLLM 推理服务（模型就绪过程在容器后台继续）..."
...
echo "vLLM 已提交启动；LiteLLM 直连端口：${api_port}，DP 健康检查端口：${service_port}"
