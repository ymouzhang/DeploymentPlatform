...
./dp-litellm-config --output images config/config.json >"${configured_refs}"
...
if [ -f "${archive}" ]; then
  ...
  if ! cmp -s "${configured_refs}" "${bundled_refs}"; then
    ...
    echo "离线环境不允许修改 litellm_image 或 postgres_image" >&2
    exit 1
  fi
  ...
  if [ "${need_load}" -eq 1 ]; then
    echo "正在加载 LiteLLM 离线镜像..."
    docker image load --input "${archive}"
  else
    echo "LiteLLM 离线镜像均已加载，跳过重复导入"
  fi
