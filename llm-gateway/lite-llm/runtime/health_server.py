#!/usr/bin/env python3
"""Translate LiteLLM readiness into DP's exact /healthz contract."""
...
class HealthHandler(BaseHTTPRequestHandler):
    ...
    do_POST = do_PUT = do_PATCH = do_DELETE = do_OPTIONS = do_HEAD = lambda self: self.send_error(
        405, "health server does not proxy gateway requests"
    )

    def dp_health(self):
        connection = http.client.HTTPConnection(self.upstream_host, self.upstream_port, timeout=3)
        try:
            connection.request("GET", "/health/readiness", headers={"Accept": "application/json"})
