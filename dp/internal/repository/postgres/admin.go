...
func (db *DB) DashboardMetrics(
	...
	err := db.pool.QueryRow(ctx, `SELECT
		...
		(SELECT count(*) FROM models WHERE deleted_at IS NULL AND status = 'ready'),
		(SELECT count(*) FROM models WHERE deleted_at IS NULL AND status IN ('uploading', 'deploying', 'deleting')),
		(SELECT count(*) FROM models WHERE deleted_at IS NULL AND status = 'failed'),
		...
		&metrics.Models,
		&metrics.ReadyModels,
		&metrics.ProcessingModels,
		&metrics.FailedModels,
