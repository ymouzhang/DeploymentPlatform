...
主机仅描述 SSH 端点，不包含服务类型、安装目录或安装状态。唯一业务键为 `(owner_id, ip, ssh_port)`。新建主机时必须先完成 SSH、SFTP、架构和主机指纹校验，再原子保存主机与校验结果；修改 IP、端口、SSH 用户或密码时同样先校验新连接，失败不修改原记录。
...
2. 用户注册主机；DP 在保存前完成 SSH 与 SFTP 校验，并同时保存主机指纹、架构和校验时间。
