...
- `actor_roles` 是稳定的必填数组；匿名认证事件返回空数组 `[]`，不得省略或返回 `null`。前端在 API 边界和渲染层仍按空数组兜底，避免单条历史数据破坏整页。
...
- 应用 Shell 和每个业务页面之间设置错误边界。单页渲染异常只替换当前内容区，必须保留导航，并提供“重试”和“返回首页”；首页按当前账号首个可访问页面选择，路由变化自动清除上一页错误状态。
- 浏览器标题随当前路由更新为 `DP · 页面名称`，未知路由使用 `DP · 部署平台`。
- 推荐 Query Key：
...
保存完整副本。上传完成后 DP 自动缓存固定摘要校验器并在目标机本地完成摘要和 tar 安全校验，按压缩
字节数回传进度；工具不可用时回退到并发 SFTP 流式读取。远端解压到同盘临时
