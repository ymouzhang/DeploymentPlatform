...
const operation = { id: 'op', service_instance_id: 'env', owner_id: 'owner', action: 'install', status: 'failed', stage: 'package', created_at: '2026-01-01T00:00:00Z' } satisfies Operation
...
describe('remediationLink', () => {
  ...
  it('routes install failures to the exact service instance', () => {
    expect(remediationLink({ ...operation, error_code: 'SCRIPT_FAILED' })).toBe('/services?owner_id=owner&service_instance_id=env')
  })
})
