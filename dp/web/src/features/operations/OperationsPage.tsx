    ...
    { title: '所属账号', dataIndex: 'owner_username', width: 130, render: (v) => <Tag>{v || '-'}</Tag> },
    {
      ...
      render: (_, item) => (
        <div className="operation-target-cell">
          <Typography.Text strong ellipsis={{ tooltip: item.environment_name || item.environment_id }}>
            {item.environment_name || item.environment_id}
          </Typography.Text>
          <Typography.Text
            type="secondary"
            className="cell-caption"
            ellipsis={{ tooltip: `${item.environment_ip} · ${item.service_type}` }}
          >
            {item.environment_ip} · {item.service_type}
          </Typography.Text>
        </div>
      ),
    },
