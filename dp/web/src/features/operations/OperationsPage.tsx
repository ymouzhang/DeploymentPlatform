...
export function OperationsPage() {
  ...
  const query = useInfiniteQuery({
    ...
    initialPageParam: '',
    queryFn: ({ pageParam }) => api.listOperations({ ...filter, cursor: pageParam || undefined }),
  ...
  const columns = useMemo<ColumnsType<Operation>>(() => [
    ...
    { title: '所属账号', dataIndex: 'owner_username', width: 130, render: (v) => <Tag>{v || '-'}</Tag> },
    {
      ...
      width: 240,
      render: (_, item) => (
        <div className="operation-target-cell">
          <Typography.Text strong ellipsis={{ tooltip: item.service_instance_name || item.service_instance_id }}>
            {item.service_instance_name || item.service_instance_id}
          </Typography.Text>
          <Typography.Text
            type="secondary"
            className="cell-caption"
            ellipsis={{ tooltip: `${item.host_ip} · ${item.service_type}` }}
          >
            {item.host_ip} · {item.service_type}
          </Typography.Text>
        </div>
      ),
    },
    ...
    { title: '操作', width: 240, fixed: 'right', render: (_, item) => <Space><Button icon={<EyeOutlined />} onClick={() => setSelected(item.id)}>日志</Button><Button onClick={() => navigate(`/audit?operation_id=${item.id}`)}>审计</Button>{item.status !== 'succeeded' && <Button onClick={() => navigate(remediationLink(item))}>去处理</Button>}</Space> },
  ], [navigate])
...
function failureAdvice(item: Operation) { if (item.error_code?.includes('PACKAGE')) return '检查对应账号的安装包'; if (item.error_code?.includes('TIMEOUT')) return '检查 SSH 连通性和远端脚本'; if (item.error_code?.includes('PASSWORD') || item.error_message?.includes('SSH')) return '重新校验 SSH 凭据'; if (item.action === 'install') return '检查日志后重置残留状态'; return '查看日志并检查目标服务实例' }
export function remediationLink(item: Operation) { if (item.error_code?.includes('PACKAGE')) return `/packages?owner_id=${item.owner_id ?? ''}`; if (item.action === 'install' && !item.error_code?.includes('SSH')) return `/services?owner_id=${item.owner_id ?? ''}&service_instance_id=${item.service_instance_id}`; return `/hosts?owner_id=${item.owner_id ?? ''}` }
