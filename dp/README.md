...
上传安装包时可选择已有服务类型或创建自定义类型；支持多个安装包在浏览器后台并发上传并分别展示进度，上传结束后自动进入服务端校验。每次上传形成不可变版本，可查看引用、切换当前版本并按保留策略清理安全的旧版本。
...
模型管理支持选择已有主机并复用 SSH 凭据，将多个超大 `.tar.gz` 按独立会话并发分片中转到目标机，支持断点续传、
