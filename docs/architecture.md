...
- 文件系统按账号保存当前安装包，操作日志通过环境归属间接关联账号。
...
- 资源交接在单个 SQLite 事务中先校验源/目标账号、运行中操作和唯一键冲突，再更新 `packages.owner_id`、`package_versions.owner_id` 与 `environments.owner_id`。任何检查或更新失败均回滚。
- 安装包文件路径包含原 owner ID，交接时移动每个服务类型的完整版本目录，并同步更新当前投影和全部版本的 `storage_path`。文件移动和数据库事务采用“目标冲突预检 → 文件移动 → 数据库事务 → 失败时文件回滚”的补偿流程，并在包管理器互斥锁内执行。
...
- 安装包上传先完成临时文件，再原子替换，最后更新数据库。
- 本地业务变更先原子提交，成功审计随后追加；审计写入失败记录应用错误日志，不撤销已经完成的远端副作用。删除业务资源不级联删除审计快照。
- 删除安装包先删除数据库元数据，再删除磁盘包目录；磁盘清理失败时接口返回错误，但数据库记录已删除，残留目录不影响后续同名类型重新上传（上传为覆盖语义）。
- 删除环境在单事务中删除环境行，`service_configs` 和 `service_config_revisions` 外键级联；`operations` 与 JSONL 日志继续保留至运维历史保留任务到期。
- 已安装实例的配置先原子替换远端文件，再更新本地实例配置和健康端口。
- 如果文件替换成功但数据库提交失败，启动时对包文件重新计算摘要并修复元数据。
...
- 不启用跨域；变更类请求校验 `Origin` 与当前 Host 同源；会话 Cookie 使用 `HttpOnly`、`SameSite=Strict`，HTTPS 下使用 `Secure`。
...
- 同步业务变更与审计原子提交，异步操作 requested/completed 正确关联 operation ID。
...
- 资源交接的运行中操作、同名安装包和环境唯一键冲突测试；文件移动失败和数据库失败均验证无部分交接，历史操作归属快照保持不变。
